...
if(m_ctrls.on_mouse_button_down(m_specific->m_cur_x,
    ...
    bool platform_support::load_img(unsigned idx, const char* file)
    {
        if(idx < max_images)
        {
            ...
            printf("tmp format Amask %x\n", tmp_surf->format->Amask);
            printf("tmp format Ashift %x\n", tmp_surf->format->Ashift);
            printf("tmp format Rmask %x\n", tmp_surf->format->Rmask);
            printf("tmp format Rshift %x\n", tmp_surf->format->Rshift);
            printf("tmp format Gmask %x\n", tmp_surf->format->Gmask);
            printf("tmp format Gshift %x\n", tmp_surf->format->Gshift);
            printf("tmp format Bmask %x\n", tmp_surf->format->Bmask);
            printf("tmp format Bshift %x\n", tmp_surf->format->Bshift);
           // tmp_surf->format->Ashift = 24; 
            ...
            m_specific->m_surf_img[idx] = tmp_surf;
    ...
    void platform_support::on_multi_gesture(float dTheta, float dDist, float x, float y, int numFinders) {}
